<?php
   ////////////////////////////////////////////
   // //Include the following line only if running this page directly and from another page
   // include 'CoreFunctions.php';
   ////////////////////////////////////////////

   ////////////////////////////////////////////
   // //Individual Function Tests
   // //Simply uncomment each function to test individual function separately
   ...
   ////////////////////////////////////////////
   // //Uncomment the following lines when testing individual functions
   ...
   ////////////////////////////////////////////
   // //Return list of user_id to be populated into a dropdown list as input
   function getUserList(){
      // //Connects to local capstone database
      $conn = connectToLocal('capstoneLocal');
      // //Retrieve all user_id from the users table that are currently in the local DB
      $query = "select id from users order by id asc";
      // //Results of query stored into $useridList
      $useridList = getResult($conn, $query);

      // //If the result object is not empty, generate form HTML tag to create dropdown menu
      if($useridList->num_rows > 0){
         echo "Total Users: " . $useridList->num_rows . "<br>";
      ?>
      <!--We are posting to PHP_SELF for now to verify dropdown menu actually works-->
         <form name="form1" method="POST" action="<?php $_SERVER['PHP_SELF'];?>">
         <select name="user_id">
         <option value=""></option> <!--if you want an empty value as the top of the dropdown list, otherwise may be commented out-->
      <?php
   ...
   ////////////////////////////////////////////
   // //Compute the number of invocations between the date 2016-01-01 to 2016-01-25 (or any given two dates) on a per user base
   function invocationsPerUser(){
      
      //Connects to local DB
      $conn = connectToLocal('capstoneLocal');
      //Query for all current user_id from local users table
      $query = "select id from users order by id asc";
      //Stored result object in $useridList
      $useridList = getResult($conn, $query);
      $startDate = '2016-01-01';
      $endDate = '2016-01-25';

      //Initialize JSON variable with chart specifications, customization is set here, which will be use to draw the chart object
      $arrData = array(
         "chart" => array(
      ...
      // //Dataset holds the group, or set, of data that will be displayed for each categories
      $arrData['dataset'] = array();
      // //Categories defines the category types that we want to differentiate on the graph
      $arrData['categories'] = array();
      // //Query for the 4 possible types of result you can have from the invocations table
      $query = "select distinct result from invocations";
      // //Store result object in $resultTypes that will later be used to generate the categories array
      ...
      if($useridList->num_rows > 0){
         ...
         // //Array that will hold all 4 types of result from invocations table 
         $category = array();

         //We would like to query for the count for each type of result from the invocations table according to each user_id
         foreach($resultTypes as $type){
            // array_push($arrData['dataset'], array('seriesname'=>$type['result'], 'data'=>array()));
            // //Array which stores the data of a given type of result per user_id
            $data = array();
            // //Loop through every user_id for all 4 types of result
            foreach($useridList as $user){
               ...
               // echo $query . "<br>";
               // //Returns all invocations of a user_id for a given result type
               $invocationEvents = getResult($conn, $query);
               // printResultInTable($invocationEvents);
               //creates array to hold invocation data 
               // $data = array();

               // //Push the number of invocations of a given result type into array $data if there is result, else push null
               ...
               // //Creates the label with user_id for each type of result and store them into $category array
               if (empty($arrData['categories']))
                  array_push($category, array('label' => $user['id']));
            }
            // //Once all user_id has been looped through to obtain invocations count for a given result type, 
            // //push those arrays into the dataset index of the JSON variable
            ...
            // //
            if (empty($arrData['categories'])){
               // //Push the $category array into $arrData JSON variable 
               array_push($arrData['categories'], array('category' => $category));
            }
         }
         ...
         // //Encode php chart data into JSON format
         $jsonEncodedData = json_encode($arrData, true);
         // //Create chart object with the JSON data
   ...
   /////////////////////////NOT USED/////////////////////////
   // //Compute the number of times a file is compiled between the date 2016-01-01 to 2016-01-25 (or any given two dates) on a per user base
   ...
   function invokeExceptions(){
      // //Connect to local DB
      $conn = connectToLocal('capstoneLocal');
      // //Select all user_id from local users table
      $query = "select id from users order by id asc";
      $useridList = getResult($conn, $query);
      $startDate = '2016-01-01';
      $endDate = '2016-01-25';
